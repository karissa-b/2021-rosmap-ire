...
Therefore, I will follow the [edgeR users guide](https://www.bioconductor.org/packages/release/bioc/vignettes/edgeR/inst/doc/edgeRUsersGuide.pdf) for removing batch effects. This method uses an additive model formular to block the effect of batch, then test for differential expression between groups of interest. 
...
### Visualiastions 
```{r fig.cap="Volcano plot of gene expression in male and female AD subjects relative to no cognitive impairment controls. No genes are found to be significantly DE (at FDR < 0.05)"}
toptable_1 %>% 
  bind_rows(.id = "coef") %>% 
...
names(diagColours) <- levels(dge$samples$diagnosis)
```
...
### Visualiastions 
```{r fig.cap="Volcano plot of gene expression in male and female AD subjects relative to no cognitive impairment controls after CQN. No genes are found to be significantly DE (at FDR < 0.05)"}
toptable_cqn %>% 
  bind_rows(.id = "coef") %>% 
  ...
  scale_color_continuous(type = "viridis", end = 0.9, direction = -1)
```

```{r fig.cap="MD plot of gene expression in male and female AD subjects relative to no cognitive impairment controls after CQN. No genes are found to be significantly DE (at FDR < 0.05)"}
toptable_cqn %>% 
  bind_rows(.id = "coef") %>% 
