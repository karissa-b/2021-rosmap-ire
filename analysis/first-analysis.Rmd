...
No evident changes were observed to GC content after trimming and quality filtering.
...
Libary sizes vary considerably, which appears to be due to samples being sequenced across multiple runs. Library sizes range between  `r pander(comma(range(dge$samples$lib.size)))`. Although concerning, this will be accounted for using  `TMM normalisation`.
```{r}
dge$samples %>% 
