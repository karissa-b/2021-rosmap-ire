...
[Lagomarsino et al. 2021](https://doi.org/10.1016/j.neuron.2021.08.003) noted in their methods section that a few samples may show abnormally high levels of Oct2, Oct3 or LEFTY2. This implies that these cell lines may contain a high proportion of undifferentiated cells and should be removed. These three genes were filtered from the analysus in the filtering step, meaning they are lowly expressed and considered undetectable. 

The graphs below show the expression of these genes from the unfiltered expression levels. A few of the cell lines show some expression of LEFTY2. However, I am unsure at this stage whether the expression is high enough to consider the cell line as undifferentiated. I will note the sample IDs for now, and assess whether they appear to be outliers when performing differential expression analysis. 
...
featureCounts %>% 
  ...
  as_tibble() %>% 
  left_join(meta) %>% 
  ggplot(aes(x = sample, y = logCPM, fill = sample %in% samps2keep)) +
  geom_col()+
...
featureCounts %>% 
  ...
  geom_col(aes(alpha = sample %in% samps2keep,
               fill = rnaBatch))+
  facet_wrap(~gene_name) +
  easy_rotate_x_labels(angle = -45) 
  #theme(legend.position = "none")
...
Might be a trend happening between the second pricipal component and the RIN of the sample. 
