...
Still playing around with this. Maybe try RUV?
```{r}
# define the batch effect to remove. 
batch <- dge$samples$libraryBatch

# define the null model matix, which only contains an intercept
adjustedCounts <- ComBat_seq(dge$counts, 
           batch = batch, 
           covar_mod  = dge$samples %>% 
             mutate(Braak = as.factor(Braak), 
                    CERAD = as.factor(CERAD)) %>% 
             dplyr::select(diagnosis, sex, apoeGenotype, yearsEducation, CERAD, Braak) %>%
             as.data.frame()
           )
